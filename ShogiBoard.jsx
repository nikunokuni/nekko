...
export default function ShogiBoard({ board: boardProp, stamps: stampsProp = [], onChange, readOnly = false }) {
  ...
  // BUG FIX ①: boardProp の参照が変わっても内容が同じ場合、
  // useEffect が毎レンダーで setBoard を呼んで無限ループになりうる。
  // JSON 文字列で比較して実際に変化したときのみ更新する。
  ...
  // BUG FIX ②: stampsProp も同様に参照比較では毎回更新される。
  ...
  // BUG FIX ③: 駒を「後手（大文字）」と判定する条件が逆。
  // 将棋の慣例では先手（下側）が小文字、後手（上側）が大文字とする設計だが、
  //   `p === p.toLowerCase()` → 小文字なら true → isSente = true （先手）
  // これは p が数字・記号のとき常に true になるため、
  // 半角アルファベットのみを対象に判定する。
  ...
  const draw = useCallback(() => {
    ...
    for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        const p = board[r]?.[c]; // BUG FIX ④: board[r] が undefined の場合クラッシュ防止
        if (!p || p === ' ') continue;
    ...
    for (const st of stamps) {
      // BUG FIX ⑤: st.type が STAMP_COLOR にないキーだと fillStyle が undefined になる。
  ...
  const handleClick = useCallback((e) => {
    ...
    if (selected) {
      const from = selected;
      setSelected(null);
      if (from.row === row && from.col === col) return; // 同じマスは何もしない
  ...
  // BUG FIX ⑥: タッチ操作（スマホ）でキャンバスをタップしたとき
  // onClick が発火しないブラウザがある。onTouchEnd で補完する。
  const handleTouchEnd = useCallback((e) => {
    if (readOnly || !board) return;
    // シングルタップのみ対応（マルチタッチは無視）
