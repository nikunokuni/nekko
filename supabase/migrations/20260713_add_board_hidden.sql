-- ══════════════════════════════════════════════════
-- 盤面の「非表示」状態を保存する board_hidden カラムを追加
--   これまで非表示はローカルstateのみで、ノードを開き直すと
--   盤面が再表示されてしまっていた。
--   あわせて copy_tree RPC にも board_hidden のコピーを追加する。
-- Supabase の SQL Editor で実行してください
-- ══════════════════════════════════════════════════

alter table public.nodes
  add column if not exists board_hidden boolean not null default false;
...
begin
  ...
  values (v_user_id, coalesce(p_new_name, v_src.name || '（コピー）'), coalesce(v_src.tags, '{}'::text[]), true)
