-- ══════════════════════════════════════════════════
-- ノードに「いつ使う」「序盤の意識」を追加
--   when_to_use:   きほんの短文メモ。どんなときに使う戦法・局面かを一言で。
--                  親から子ノード一覧を見たとき、各子の下に表示される。
--   opening_focus: 「ついか」の項目。この局面へ持っていくために序盤で意識すべきこと。
--                  設定（tsuika_visibility）で表示ON/OFFできる。
...
alter table public.nodes
  add column if not exists opening_focus text;
...
begin
  ...
  insert into public.nodes (
    ...
    turn, evaluation, when_to_use, opening_focus
  )
  select
    ...
    n.turn, n.evaluation, n.when_to_use, n.opening_focus
  from public.nodes n
