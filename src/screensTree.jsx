...
// ──────────────────────────────────────────────────
// InputField: ラベル付き入力フィールド（共通パーツ）
// ──────────────────────────────────────────────────
function InputField({ label, value, onChange, placeholder }) {
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ fontSize: 11, color: "rgba(26,15,0,0.5)", marginBottom: 5 }}>
        {label}
      </div>
      <input
        value={value}
...
function CreateTreeModal({ onClose, onCreate }) {
  ...
  const handleCreate = () => {
    ...
    onCreate(newName.trim(), parseTags(newTags));
    onClose();
  };
  ...
  return (
    <div style={MODAL_OVERLAY_STYLE} onClick={onClose}>
      <div style={MODAL_SHEET_STYLE} onClick={(e) => e.stopPropagation()}>
        <div style={{ fontFamily: "'Shippori Mincho B1',serif", fontSize: 16, color: "#1a0f00", marginBottom: 20 }}>
          新しいツリーを作成
        </div>
        <InputField label="戦法名" value={newName} onChange={setNewName} placeholder="例：中飛車" />
        <InputField label="タグ（カンマ区切り）" value={newTags} onChange={setNewTags} placeholder="例：振り飛車, 中飛車" />
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} style={BTN_CANCEL_STYLE}>キャンセル</button>
          <button
            onClick={handleCreate}
            disabled={!newName.trim()}
            style={{
              flex: 2, padding: 12, borderRadius: 12, border: "none", fontSize: 13, fontWeight: 600,
              cursor: newName.trim() ? "pointer" : "default",
              background: newName.trim() ? "#a07840" : "#B4B2A9",
...
function EditTreeModal({ tree, onClose, onSave }) {
  ...
  const [editTags,   setEditTags]   = useState((tree.tags || []).join("、"));
  const [editActive, setEditActive] = useState(tree.active);
  const [saving,     setSaving]     = useState(false);

  const handleSave = async () => {
    if (!editName.trim()) return;
    setSaving(true);
    await onSave(tree.id, { name: editName.trim(), tags: parseTags(editTags), active: editActive });
    setSaving(false);
    onClose();
  };
  ...
  return (
    <div style={MODAL_OVERLAY_STYLE} onClick={onClose}>
      <div style={MODAL_SHEET_STYLE} onClick={(e) => e.stopPropagation()}>
        ...
        <InputField label="戦法名"                  value={editName} onChange={setEditName} placeholder="例：中飛車" />
        <InputField label="タグ（カンマ区切り）" value={editTags} onChange={setEditTags} placeholder="例：振り飛車, 中飛車" />
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button
            ...
            style={{
              ...
              background: editName.trim() && !saving ? "#a07840" : "#B4B2A9",
              color: "#faf4e8", fontFamily: "'Noto Serif JP',serif",
            }}
          >
            {saving ? "保存中..." : "保存する"}
...
function DeleteTreeModal({ tree, onClose, onConfirm }) {
  ...
  return (
    <div
      style={{ position: "absolute", inset: 0, background: "rgba(26,15,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: "20px" }}
      onClick={onClose}
    >
      <div style={{ width: "100%", maxWidth: 360, background: "#faf4e8", borderRadius: 20, padding: "28px 24px" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button
            ...
            style={{
              ...
              background: deleting ? "#B4B2A9" : "#A93226",
              color: "#faf4e8", fontFamily: "'Noto Serif JP',serif",
            }}
          >
            {deleting ? "削除中..." : "削除する"}
    ...
    <div style={{ position: "relative", marginBottom: 10 }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          <span style={{ fontSize: 15, fontWeight: 600, color: "#1a0f00", fontFamily: "'Shippori Mincho B1',serif", flex: 1 }}>
            {tree.name}
          </span>
          <span style={{
          ...
          {tree.is_public && (
            <span style={{ fontSize: 10, padding: "3px 8px", borderRadius: 10, background: "#EAF3DE", color: "#3B6D11", fontFamily: "'Noto Serif JP',serif" }}>
              公開中
            </span>
          )}
          <button
            onClick={handleMenuToggle}
            style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(26,15,0,0.35)", fontSize: 16, padding: "2px 4px", borderRadius: 6, lineHeight: 1 }}
          >
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
          {(tree.tags || []).map((t) => (
            <span key={t} style={{ fontSize: 10, padding: "2px 7px", borderRadius: 8, background: "rgba(26,15,0,0.06)", color: "rgba(26,15,0,0.5)", fontFamily: "'Noto Serif JP',serif" }}>
              {t}
            </span>
          ))}
      ...
      {menuOpen && (
        <>
          ...
          <div style={{
            ...
            <div
              onClick={handleEdit}
              style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 16px", fontSize: 13, cursor: "pointer", color: "#1a0f00", fontFamily: "'Noto Serif JP',serif" }}
            ...
            <div
              ...
              style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 16px", fontSize: 13, cursor: "pointer", color: "#A93226", fontFamily: "'Noto Serif JP',serif" }}
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#faf4e8" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 18px 12px", borderBottom: "0.5px solid rgba(26,15,0,0.12)" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <button onClick={onPublic}                    style={{ background: "none", border: "none", cursor: "pointer", color: "#a07840", fontSize: 20, padding: 2 }}><i className="ti ti-world" /></button>
          <button onClick={() => setShowCreateModal(true)} style={{ background: "#a07840", border: "none", cursor: "pointer", color: "#faf4e8", fontSize: 13, padding: "6px 14px", borderRadius: 10, fontFamily: "'Noto Serif JP',serif", display: "flex", alignItems: "center", gap: 4 }}>
            <i className="ti ti-plus" style={{ fontSize: 13 }} /> 新規
          </button>
          <button onClick={onSignOut}                   style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(26,15,0,0.3)", fontSize: 18, padding: 2 }}><i className="ti ti-logout" /></button>
        </div>
      </div>
          ...
          <>
            {active.length > 0 && (
              <>
                ...
                {active.map((t) => <TreeCard key={t.id} tree={t} onOpen={onOpen} onEdit={setEditTarget} onDelete={setDeleteTarget} />)}
              </>
            )}
            {inactive.length > 0 && (
              <>
                <div style={{ fontSize: 10, color: "rgba(26,15,0,0.4)", letterSpacing: "0.1em", marginBottom: 8, marginTop: active.length > 0 ? 16 : 0 }}>休止中</div>
                {inactive.map((t) => <TreeCard key={t.id} tree={t} onOpen={onOpen} onEdit={setEditTarget} onDelete={setDeleteTarget} />)}
              </>
            )}
          </>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {children.map((child) => {
              ...
              return (
                <div
                  key={child.id}
                  onClick={() => saveAndGo(() => onNodeSelect(child.id))}
            ...
            <div
              onClick={() => saveAndGo(() => onNewNode(nodeId))}
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#faf4e8" }}>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 24px", gap: 16, textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 8, width: "100%" }}>
            <button onClick={onCancel} style={{ flex: 1, padding: 10, borderRadius: 10, fontSize: 12, cursor: "pointer", background: "#faf4e8", color: "rgba(26,15,0,0.5)", border: "0.5px solid rgba(26,15,0,0.18)", fontFamily: "'Noto Serif JP',serif" }}>
              ツリーに戻る
            </button>
            <button onClick={() => newNodeId ? onOpenNode(newNodeId) : onCancel()} style={{ flex: 2, padding: 10, borderRadius: 10, fontSize: 13, cursor: "pointer", background: "#a07840", color: "#faf4e8", border: "none", fontFamily: "'Noto Serif JP',serif", fontWeight: 600 }}>
              ノードを開く
            </button>
          </div>
        </div>
