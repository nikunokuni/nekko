// ══════════════════════════════════════════════════
// data.js  ―  ねっこ  定数 / メタ / サンプルデータ
// ══════════════════════════════════════════════════

// ── 将棋の駒ラベル ───────────────────────────────
export const PIECE_LABEL = {
  k:'玉', r:'飛', b:'角', g:'金', s:'銀', n:'桂', l:'香', p:'歩',
  K:'王', R:'飛', B:'角', G:'金', S:'銀', N:'桂', L:'香', P:'歩',
};
export const PROMOTED_LABEL = {
  p: 'と', l: '杏', n: '圭', s: '全', b: '馬', r: '龍',
};
// 成れる駒の種類（小文字baseKeyで判定）
export const PROMOTABLE = new Set(['p', 'l', 'n', 's', 'b', 'r']);

// ── 初期盤面（平手）──────────────────────────────
export const INITIAL_BOARD = [
  ['L','N','S','G','K','G','S','N','L'],
  [' ','R',' ',' ',' ',' ',' ','B',' '],
  ['P','P','P','P','P','P','P','P','P'],
  [' ',' ',' ',' ',' ',' ',' ',' ',' '],
  [' ',' ',' ',' ',' ',' ',' ',' ',' '],
  [' ',' ',' ',' ',' ',' ',' ',' ',' '],
  ['p','p','p','p','p','p','p','p','p'],
  [' ','b',' ',' ',' ',' ',' ','r',' '],
  ['l','n','s','g','k','g','s','n','l'],
];

// ── ステータス表示メタ ────────────────────────────
export const STATUS_META = {
  done: { label:'完成',  bg:'#EAF3DE', color:'#3B6D11', dot:'#3B6D11' },
  wip:  { label:'研究中', bg:'#FAEEDA', color:'#7B3010', dot:'#7B3010' }, // マップのノード枠色(T.redDark)と統一
  todo: { label:'未定',  bg:'transparent', color:'#5F5E5A', dot:'#B4B2A9', dashed:true },
};

// ── 「よく使う」表示メタ（1: めったに 〜 5: いつも）──
// scale はマップ上のノードの拡大率。最大でもノード間隔（NODE_W + 16）に収まるよう
// 全体を小さめにしてあり、拡大ノード同士が重ならない（110 × 1.1 = 121 < 126）
export const USAGE_META = {
  1: { label: 'めったに',   scale: 0.7 },
  2: { label: 'たまに',     scale: 0.8 },
  3: { label: 'ふつう',     scale: 0.9 },
  4: { label: 'よく',       scale: 1.0 },
  5: { label: 'いつも',     scale: 1.1 },
};
export const USAGE_LEVELS = [1, 2, 3, 4, 5];

// ── 勝率ラジオの選択肢（割単位）──
export const WIN_RATE_LEVELS = [1, 3, 5, 7, 9];

// ── 一言コメントのプリセットグループ ────────────────
export const COMMENT_GROUPS = [
  {
    label: '感触',
    noGroupTag: true,
    items: ['気持ちよく勝てた','研究が活きた','罠にかかった','悔しい負け','良い将棋だった','展開が難しかった','定跡から外れた','逆転勝ち','逆転負け'],
  },
  {
    label: '課題',
    noGroupTag: true,
    items: ['時間切れ','読み抜け','序盤ミス','中盤ミス','終盤ミス','もっと研究が必要'],
  },
];

// ── 好き度ラジオの選択肢 ──────────────────────────
export const LIKE_LEVELS = [
  { value: 1, label: '普通' },
  { value: 2, label: 'まあまあ' },
  { value: 3, label: '好き' },
  { value: 4, label: 'かなり' },
  { value: 5, label: '大好き' },
];

// ── 志向（戦法の方向性）表示メタ ──────────────────
// マインドマップのノード・線の色分けに使用する
export const ORIENTATION_META = {
  '攻め':     { label:'攻め',     color:'#A93226', bg:'#fadbd8' },
  '受け':     { label:'受け',     color:'#1a5276', bg:'#d6eaf8' },
  'バランス': { label:'バランス', color:'#3B6D11', bg:'#EAF3DE' },
  '不明':     { label:'不明',     color:'#5F5E5A', bg:'#F1EFE8' },
};

// ── 盤面テンプレート ─────────────────────────────
// board: 9×9 配列（大文字=後手, 小文字=先手）
// 駒記号: p=歩 l=香 n=桂 s=銀 g=金 b=角 r=飛 k=玉（大文字は後手）
// hand: { p, l, n, s, g, b, r } 各駒の枚数
export const BOARD_TEMPLATES = [
  {
    name: "平手（初期配置）",
    board: INITIAL_BOARD.map(r => [...r]),
    handSente: { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
    handGote:  { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
  },
   {
     name: "ゴキ中",
     board: [
       ['L','N',' ',' ',' ','G',' ','N','L'],  // 9段目（後手）
       [' ','R',' ',' ','G',' ','K','B',' '],
       [' ',' ','P','S','P','P','S','P','P'],
       ['P',' ',' ','P',' ',' ','P',' ',' '],
       [' ','P',' ',' ','p',' ',' ',' ',' '],
       [' ',' ','p',' ','s',' ',' ',' ',' '],
       ['p','p','b','p',' ','p','p','p','p'],
       [' ',' ',' ',' ','r',' ','s','k',' '],
       ['l','n',' ','g',' ','g',' ','n','l'],  // 1段目（先手）
     ],
     handSente: { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
     handGote:  { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
   },
   {
     name: "詰将棋",
     board: [
       [' ',' ',' ',' ','K',' ',' ',' ',' '],  // 9段目（後手）
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],  // 1段目（先手）
     ],
     handSente: { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
     handGote:  { p:18, l:4, n:4, s:4, g:4, b:2, r:2 },
   },
   {
     // 先手側に金矢倉（8八玉・7八金・6七金・7七銀）を組んだ形。後手は初期配置
     name: "金矢倉",
     board: [
       ['L','N','S','G','K','G','S','N','L'],  // 9段目（後手）
       [' ','R',' ',' ',' ',' ',' ','B',' '],
       ['P','P','P','P','P','P','P','P','P'],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ','p','p','p',' ',' ',' ',' '],  // ７六・６六・５六歩
       ['p','p','s','g',' ','p','p','p','p'],  // ７七銀・６七金
       [' ','k','g','b',' ',' ',' ','r',' '],  // ８八玉・７八金・６八角・２八飛
       ['l','n',' ',' ',' ',' ','s','n','l'],  // 1段目（先手）
     ],
     handSente: { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
     handGote:  { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
   },
   {
     // 先手側に四間飛車＋本美濃（2八玉・3八銀・5八金・4九金）。後手は初期配置
     name: "美濃囲い（四間飛車）",
     board: [
       ['L','N','S','G','K','G','S','N','L'],  // 9段目（後手）
       [' ','R',' ',' ',' ',' ',' ','B',' '],
       ['P','P','P','P','P','P','P','P','P'],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ','p','p',' ',' ',' ',' ',' '],  // ７六・６六歩
       ['p','p',' ',' ','p','p','p','p','p'],
       [' ','b',' ','r','g',' ','s','k',' '],  // ８八角・６八飛・５八金・３八銀・２八玉
       ['l','n','s',' ',' ','g',' ','n','l'],  // ７九銀・４九金
     ],
     handSente: { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
     handGote:  { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
   },
   {
     // 先手側に居飛車穴熊（9九玉・9八香・8八銀・7九金・7八金・7七角）。後手は初期配置
     name: "居飛車穴熊",
     board: [
       ['L','N','S','G','K','G','S','N','L'],  // 9段目（後手）
       [' ','R',' ',' ',' ',' ',' ','B',' '],
       ['P','P','P','P','P','P','P','P','P'],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ','p',' ',' ',' ',' ',' ',' '],  // ７六歩
       ['p','p','b','p','p','p','p','p','p'],  // ７七角
       ['l','s','g',' ',' ',' ',' ','r',' '],  // ９八香・８八銀・７八金・２八飛
       ['k','n','g',' ',' ',' ','s','n','l'],  // ９九玉・７九金・３九銀
     ],
     handSente: { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
     handGote:  { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
   },
  /* ── 以下にテンプレートを追加してください ─────
   {
     name: "テンプレート名",
     board: [
       ['L','N','S','G','K','G','S','N','L'],  // 9段目（後手）
       [' ','R',' ',' ',' ',' ',' ','B',' '],
       ['P','P','P','P','P','P','P','P','P'],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       [' ',' ',' ',' ',' ',' ',' ',' ',' '],
       ['p','p','p','p','p','p','p','p','p'],
       [' ','b',' ',' ',' ',' ',' ','r',' '],
       ['l','n','s','g','k','g','s','n','l'],  // 1段目（先手）
     ],
     handSente: { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
     handGote:  { p:0, l:0, n:0, s:0, g:0, b:0, r:0 },
   },
*/
];

// ── 「ついか」欄のカスタム表示項目 ─────────────────
// ノード詳細の「ついか」内の項目は設定でON/OFFできる
// （メモアプリとして、自分に必要な評価軸だけに絞れるように）。
// OFFは表示を消すだけで、入力済みのデータは消えない。
export const TSUIKA_ITEMS = [
  { key: "orientation", label: "志向（攻め・受け・バランス）",   icon: "ti-compass" },
  { key: "usage",       label: "頻度（どのくらい指すか）",       icon: "ti-flame" },
  { key: "winRate",     label: "勝率（どのくらい勝てるか）",     icon: "ti-trophy" },
  { key: "likeLevel",   label: "好き度（どのくらい好きか）",     icon: "ti-heart" },
  { key: "studyMemo",   label: "研究メモ（狙い・注意・宿題）",   icon: "ti-notes" },
];

// ── 戦法グループ（相手の戦法・自分の戦法で共用）────
export const STRATEGY_GROUPS = [
  {
    label: '居飛車',
    items: ['矢倉','角換わり','横歩取り','相掛かり','雁木','棒銀','右四間飛車','一手損角換わり','袖'],
  },
  {
    label: '振り飛車',
    items: ['四間飛車','三間飛車','石田流','中飛車','ゴキゲン中飛車','向かい飛車','角交換振り飛車','鬼殺し','ツノ銀中飛車'],
  },
  {
    label: '囲い',
    noGroupTag: true,
    items: ['美濃囲い','高美濃','銀冠','穴熊','ダイヤモンド美濃','矢倉囲い','船囲い','金無双','左美濃'],
  },
];
