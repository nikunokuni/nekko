...
function layoutTree(nodes, rootId) {
  ...
  function calcDepth(id, depth) {
    ...
    children.forEach(cid => calcDepth(cid, depth + 1));
  }
  ...
  function assign(id, depth) {
    ...
    let end = yCounter - 1;
    const midY = ((start + end) / 2) * (NODE_H + 22);
      ...
      >
        ...
        <div style={{
          ...
          <div style={{flex:1,overflowY:'auto'}}>
            <Accordion
              nodes={nodes}
              rootChildIds={rootNode?.childIds || []}
              onSelect={jumpTo}
            />
          </div>
        </div>
      ...
      <div style={{display:'flex',gap:10,flexWrap:'wrap',padding:'8px 16px',borderTop:'0.5px solid rgba(26,15,0,0.18)',background:'#f0e8d4'}}>
        {[
          ...
          <div key={l.label} style={{display:'flex',alignItems:'center',gap:5,fontSize:10,color:'rgba(26,15,0,0.5)'}}>
            <div style={{width:18,height:2,borderRadius:1,background:l.line}}/>
            {l.label}
          </div>
        ))}
        {[
          ...
          <div key={s.label} style={{display:'flex',alignItems:'center',gap:5,fontSize:10,color:'rgba(26,15,0,0.5)'}}>
            <div style={{width:8,height:8,borderRadius:'50%',background:s.color,border: s.dashed ? '1px dashed #888':undefined}}/>
            {s.label}
          </div>
        ))}
      </div>
