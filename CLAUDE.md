...
- 検討したが今はやらないと決めた案は `docs/pending-ideas.md`
  （「なぜ保留か／どうなったら着手するか」を残す。同じ相談を一から考え直さないため）
...
kifuPosition.js  局面検索（盤に残した駒だけが一致する局面を探す）
```
...
**局面検索は「似ている」をアプリが決めない**（`kifuPosition.js`）。棋譜プレビューで見ている
局面を持ち込み、いらない駒を消すと、**残した駒だけが一致する**局面をためた棋譜から探す。
マスは「駒 → 見ない → 空」の3状態（「空」はこの画面でしか使わない駒で、DBには保存しない）。
距離で「なんとなく似た局面」をまとめないのは、なぜそのグループなのかを説明できないため
（`kifuFeatures.js` 冒頭と同じ判断）。**形を決めるのが人間、数えるのが機械。**

検索のために盤面を1マス1文字に詰めた列を持つ（`kifus.boards_packed`）。snapshots のままだと
1局面464バイト・300局で16MB あり、検索のたびには読めない。詰めれば81バイト（実測で約1/5.7）で、
開くときに一度読めば以降は端末の中だけで探せる。**先後は条件のほうを反転**して当てる
（棋譜側を正規化すると、先後が後から決まったときに詰め直しになる）。**両方の条件を
全棋譜へ無差別に当てない** ―― 自分と同じ側の棋譜に反転した条件まで当てると、
「相手の囲い」が「自分の囲い」として当たる。回帰テストは `test-harness/kifuPosition.test.mjs`
（先後の反転・1局1件・空の指定は、壊れても画面が落ちず結果だけ静かに変わる）。
...
無料プランは**7日間リクエストが無いとプロジェクトが一時停止**する。止まると復帰は
ダッシュボードからの手動操作で、利用者からは「ある日ログインできなくなったアプリ」に見える。

ログイン済みでアプリを開けば、それだけで `profiles` / `trees` / `nodes` / `kifus` に
7本ほど読みに行く（`useAuth.js` と `useTreeData.js` の初期化）ので、週1回誰かが開けば
実際には止まらない。**ただし未ログイン（ログイン画面のまま）だと DB アクセスは0件**
―― `App.jsx` はセッションが無いと `AuthScreen` を返すだけで、公開ツリーすら読まない。

利用者が開くことに賭けないよう、`.github/workflows/keepalive.yml` が1日1回
`trees` を1行読み、続けて `ping()`（`20260831_keepalive_ping.sql`）で1行だけ書き換える。
動かすには GitHub の Secrets に `SUPABASE_URL` / `SUPABASE_ANON_KEY`
（`.env` の `VITE_` 付きと同じ値）が要る。

**読むだけでは足りなかった。** 最初は `trees` を1行読むだけで、毎日 HTTP 200 が
返っていたのに、Supabase からは「7日以上まともな活動が無く、近いうちに一時停止する
予定」という予告メールが届いた（2026-08-31）。anon キーの読みは RLS に弾かれて空配列が
返るので、向こうの数え方では「十分な活動」に入らないらしい。**届いていることと、活動と
見なされることは別。** だからデータが実際に変わる書き込みを1本足してある。
`ping()` はテーブルを直接触らせない形にしてある（anon キーは公開値なので、
書き込みポリシーを置くと誰でも好きな値を入れられる口が増える）。

この処理が**緑でも安心してはいけない事故が2つ**あり、どちらも機械に見張らせている。

- **秘密が未設定**なら落とす。「見張っているつもりで何も投げていない」状態は、
  止まってから初めて分かる
- **宛先が本番と違う**なら落とす。`SUPABASE_URL` は登録したあと誰にも見えないので、
  別プロジェクトを叩いていても人間には見分けられず、緑のまま本番だけが7日で止まる。
  期待するプロジェクトID（`EXPECTED_HOST`）をワークフローに直書きして突き合わせる
  ―― この値はフロントのバンドルに埋め込まれて配布されている公開値なので、
  書いても新しく漏れるものは無い

なお GitHub は**リポジトリに60日間コミットが無いと定期実行を自動で止める**。
長く触らない時期は、Actions タブから手動実行するか一度コミットして起こし直す。
**60日は GitHub 側の話で、Supabase 側は7日のまま**（混ざりやすい）。
