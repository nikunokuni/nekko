...
`trees` を1行だけ読む。anon キーなので中身は空で返るが、欲しいのはデータではなく
「Postgres まで問い合わせが届いた」という事実のほう。動かすには GitHub の
Secrets に `SUPABASE_URL` / `SUPABASE_ANON_KEY`（`.env` の `VITE_` 付きと同じ値）が要る。
未設定なら**緑にせず落とす** ―― 「見張っているつもりで何も投げていない」状態は、
止まってから初めて分かるため。
